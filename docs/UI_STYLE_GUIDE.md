...
Primary navigation is fixed and limited to five modules:
...
| 论文 | Manuscript production chain: write, locate evidence, verify pages, audit citations, prepare submission | `projects/<project>/manuscript/writing_panel.html` |
...
The paper module always uses the same secondary navigation order when targets exist:
...
Do not put reading, system, backup, CSV/JSON, or cross-module shortcuts in paper subnavigation. Put those in page body cards or advanced actions.
...
- Border radius: about `10px` for cards and `7px` to `8px` for compact controls.
- Button height: at least `38px`, with core touch targets near `44px` when possible.
...
- CSV/JSON, gate commands, sync commands, and other low-frequency actions: place inside `.advanced-actions`.
...
- Actions: `.button`, `.button.primary`, `.button.secondary`, `.button.ghost`, `.advanced-actions`
- Guidance: `.page-guidance`, `.guidance-grid`, `.guidance-copy`
- Tables: `.table-wrap`, `.data-table`, `.evidence-table`, `.verification-table`, `.incoming-table`, `.writing-table`
- Reading content: `.md-view`, `.source-path`, `.wikilink`
- Status: `.status`, `.status-pill`, `.code-badge`, `.pass`, `.warn`, `.fail`, `.info`
...
| `projects/<project>/literature/evidence_locator_table.html` | 论文 |
| `projects/<project>/evidence/page_verification_queue.html` | 论文 |
| `projects/<project>/manuscript/writing_panel.html` | 论文 |
...
- Primary navigation has exactly five links: 总览 / 今日任务 / 阅读 / 论文 / 系统.
- 写论文、找证据、核页码 use the same paper subnavigation order.
