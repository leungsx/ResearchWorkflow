...
Project slug: `library_short_video`
...
- `submission_package/`: 由 `make submission-package PROJECT=library_short_video` 生成的投稿包，不需要手工新建。
...
make citation-audit PROJECT=library_short_video
make submission-package PROJECT=library_short_video

# Strict mode fails when evidence/citation ERROR issues remain.
make submission-package PROJECT=library_short_video STRICT=1
```

## Full-Paper Reader

```bash
make paper-reader PROJECT=library_short_video CITEKEY=<citekey> PDF=<authorized_pdf>
```
