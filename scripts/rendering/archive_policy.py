...
<head>
  ...
  <title>自动归档策略</title>
  <style>
    :root {{ --ink:#182026; --muted:#61707d; --line:#d9e0e6; --paper:#fff; --soft:#f6f8fa; --blue:#2463eb; --amber:#a15c07; }}
    * {{ box-sizing:border-box; }}
    ...
    .metric,.panel,.action {{ background:var(--paper); border:1px solid var(--line); border-radius:8px; padding:16px; }}
    ...
    @media (max-width:820px) {{ .metrics,.grid {{ grid-template-columns:1fr; }} h1 {{ font-size:28px; }} }}
  </style>
</head>
<body>
  <header>
