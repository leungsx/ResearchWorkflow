...
- CNKI human download handoff now exists. Use `make cnki-handoff PROJECT=<slug> TOPIC="<topic>" COUNT=12` to generate a browser-readable HTML checklist, canonical Markdown request, machine-readable CSV, and target incoming folder under `library/pdfs/<project>/incoming/<date>/`. The user then legally downloads CNKI full texts into that folder. Use `make cnki-intake PROJECT=<slug>` to validate file signatures, match files to citekeys, copy/rename into `library/pdfs/<project>/`, update `library/literature_matrix.csv`, and optionally build Readers with `BUILD_READERS=1`. By default the checklist excludes adjacent generic short-video papers; use `ALLOW_EXTERNAL=1` only when external comparison papers are intentionally desired.
...
- Git/GitHub remote backup now exists. `/Users/leung/ResearchWorkflow` is a local Git repository connected to the private GitHub remote `https://github.com/leungsx/ResearchWorkflow`. `make workflow-refresh-git DATE=<date> NOTE="<note>"` runs the local closeout, commits and pushes trackable text assets, refreshes Git health, then commits and pushes the final audit state. Git intentionally excludes PDFs, CAJ/KDH, zip backups, caches, preview images, office exports, raw data, and the external `tools/caj2pdf/` repository. The policy is documented in `docs/GIT_BACKUP_STRATEGY.md`.
...
- 2026-06-29 Git backup follow-up connected the workflow to private GitHub remote `leungsx/ResearchWorkflow`, added `scripts/git_snapshot.py`, added `make workflow-refresh-git`, documented the split backup policy, and removed the accidental `tools/caj2pdf` gitlink from the main repository index without deleting local files.
- 2026-06-30 CNKI replenishment workflow changed to human-in-the-loop by default: Codex prepares CNKI search terms, ranked download checklist, HTML page, CSV, and target folder; the user downloads through legal CNKI access; Codex ingests, validates, renames, updates matrix, builds readers, then recommends/analyzes. The current `library_short_video` request is `vault/15_CNKI_Frontier/download_requests/library_short_video/2026-06-30-library_short_video-cnki-download-request.html`.
...
- `/Users/leung/ResearchWorkflow/docs/GIT_BACKUP_STRATEGY.md`
...
- `/Users/leung/ResearchWorkflow/scripts/cnki_human_download_handoff.py`
...
- `/Users/leung/ResearchWorkflow/scripts/git_snapshot.py`
