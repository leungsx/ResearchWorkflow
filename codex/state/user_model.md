...
- The user values workflow health, backup, archive, and traceability as first-class features. After user-facing or evidence-state changes, prefer `make workflow-refresh-git DATE=<date> NOTE="<note>"` when remote backup is desired so dashboard, graph, local zip backups, file sweep, compact context, audit, Git commits, and GitHub push stay synchronized; use `make workflow-refresh` for local-only refresh.
- The user understands and accepts the split backup policy: Git/GitHub tracks text, structure, scripts, notes, CSV registries, dashboards, and history; PDFs, CAJ/KDH, zip backups, caches, preview images, office exports, raw data, and external tool repositories should stay outside Git.
